  ...
  return (
    <section
      ...
      className={`relative bg-brand-dark border-t border-white/5 ${
        isDesktop ? 'h-[300vh]' : 'h-auto py-24'
      }`}
    >
      <div
        className={`${
          isDesktop
            ...
            : 'container mx-auto px-6 overflow-visible'
        }`}
      >
        {/* 
            Conditional Layout:
        ...
        >
          ...
          <div className={`${isDesktop ? 'w-[30vw] shrink-0' : 'w-full mb-12'}`}>
            <h2 className="text-5xl md:text-8xl font-display font-bold text-white tracking-tighter mb-6 md:mb-8">
              {t('work.title')}{' '}
              <span className="text-brand-accent text-lg align-top">{t('work.number')}</span>
            </h2>
          ...
          {PROJECTS.map((project) => (
            <div
              key={project.id}
              className={`${
                isDesktop ? 'w-[45vw] shrink-0' : 'w-full'
            ...
            >
              ...
              <div className="aspect-[16/9] w-full bg-brand-gray/50 overflow-hidden mb-6 md:mb-8 relative rounded-sm">
                ...
                <img
                  ...
                  alt={`${t(project.title)} - ${t(
                    project.category
                  )} project developed with ${project.tags.join(', ')}`}
              ...
              <div className="md:pr-8">
                <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-4">
                  <span className="text-brand-accent font-mono text-[10px] md:text-xs uppercase tracking-wider">
                  ...
                  <div className="flex gap-2">
                    {project.tags.slice(0, 2).map((tag) => (
                      <span
                        key={tag}
                        className="text-[10px] uppercase border border-white/10 px-2 py-0.5 rounded-full text-white/40"
                      >
                        {tag}
                      </span>
                    ))}
